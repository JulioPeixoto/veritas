...
class DocsService:
    ...
    def _extract_text_from_pdf(self, content: bytes) -> str:
        try:
            ...
            if not text_parts:
                raise Exception("Não foi possível extrair texto de nenhuma página do PDF")
    ...
    def _extract_text_from_file(self, content: bytes, file_format: DocsType, filename: str) -> str:
        try:
            if file_format == DocsType.PDF:
                ...
                for encoding in encodings:
                    try:
                        ...
                        if len(text.strip()) > 0 and not any(char in text for char in ['\ufffd', '\x00']):
                            return text
...
{chunk}"""
    ...
    async def search_docs_with_context(self, query: str, limit: int = 5) -> Dict[str, Any]:
        ...
        for result in results:
            context_parts.append(f"[{result['document_name']} - Chunk {result['chunk']}]: {result['content']}")
    ...
    async def clear_database(self) -> Dict[str, Any]:
        try:
            self.llm_client._db = None
            
            if os.path.exists(self.llm_client.db_file):
                os.remove(self.llm_client.db_file)
                return {
                    "message": "Banco de dados limpo com sucesso",
                    "status": "cleared",
                    "file_deleted": self.llm_client.db_file
                }
            else:
                return {
                    "message": "Banco de dados já estava vazio",
                    "status": "already_empty",
                    "file_path": self.llm_client.db_file
                }
        except Exception as e:
            return {
    ...
    async def get_database_stats(self) -> Dict[str, Any]:
        try:
            if self.llm_client._db is None:
                return {"status": "empty", "total_documents": 0}
            
            all_results = self.llm_client.similarity_search("documento", k=100)
            ...
            for doc in all_results:
                ...
                if filename:
                    if filename not in unique_files:
                        unique_files[filename] = {"type": doc_type, "chunks": 0}
